...
Note: this lab was pretty much solved in the lab before so I just leave everything as it is even though it is a copy of lab 14. It can be found here:
[lab 14](14_sqli-Blind-sqli-with-time-delays.md)
...
I built a small python script that takes the whole request + the sites url and prints the admin password based on this query:

```sql
"LHDjcq5EW7cCRQxP'||(SELECT CASE WHEN SUBSTR(password,1, 1)>'g' THEN pg_sleep(3) ELSE pg_sleep(0) END FROM users WHERE username='administrator')||'"
```

A bit of binary search and splitting the whole request into smaller strings is all this python script does basically:
...
The password printed is: `r9sojzwy9f0jtsr6fbhp`
Logging in as an admin with this password worked, so it is confirmed.
