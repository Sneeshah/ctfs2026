...
#### Shell analysis
```
no directories or files only .cache and .profile
using cd throws rbash restricted errors.
rbash means this is a restricted bash shell
```
...
which flaghasher
```
returns a path for the binary so it exists which is a good start
```
/usr/local/bin/flaghasher
```
https://www.verylazytech.com/linux/bypassing-bash-restrictions-rbash seems like a good start to learn how to bypass the rbash-problem

And WOW. Just typing in 'bash' like instructed launches a normal shell and we get a hash.

The given hash is '71c3e1587669516d6e4e337cad61165f'. 
